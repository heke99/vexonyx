-- Marketing exports use the same leased/retryable operations queue as other background work.
-- Add a dedicated queue name so the export worker never claims unrelated maintenance tasks.
...
as $$
begin
  if p_queue_name not in ('inference','file-processing','embedding','sandbox','reports','email','usage','maintenance','marketing') then
