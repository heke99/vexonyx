...
begin
  ...
  v_idempotency := 'file-process:'||p_file_id::text;
...
revoke all on function artifacts.finalize_upload(uuid,uuid,uuid) from public,anon;
grant execute on function artifacts.finalize_upload(uuid,uuid,uuid) to authenticated,service_role;
