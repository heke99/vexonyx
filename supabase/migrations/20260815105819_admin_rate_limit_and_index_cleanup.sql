-- Production-aligned admin authentication rate limiting and index cleanup.
...
begin
  ...
  set window_started_at=case when v_reset then v_now else window_started_at end,
      ...
      locked_until=case when v_next_count > p_max_attempts then v_now + make_interval(secs=>p_window_seconds) else null end,
      updated_at=v_now
  where scope=p_scope and subject_hash=p_subject_hash;
...
create or replace function security.clear_admin_auth_rate_limit(p_scope text,p_subject_hash text)
returns void
language sql
security definer
set search_path=''
...
-- Remove the redundant conversation/created index that was superseded by generation_conversation_idx.
