...
export default async function Page() {
  ...
  return <div className="app-content">
    ...
    <section className="workspace-card"><header><h2>All findings</h2><span>{data?.length ?? 0} total</span></header>
      {error ? <div className="empty-state"><div><b>Findings could not be loaded.</b><p>Try again. Existing review states remain unchanged.</p></div></div> : data?.length ? data.map((finding) => <Link className="project-row" href={`/app/findings/${finding.id}`} key={finding.id}><div><b>{finding.title}</b><small>{String(finding.severity).toUpperCase()} · {finding.affected_asset || "Asset not specified"} · {finding.confidence == null ? "confidence pending" : `${Math.round(Number(finding.confidence))}% confidence`}</small></div><span>{String(finding.status).replaceAll("_", " ")} →</span></Link>) : <div className="empty-state"><div><b>No findings yet.</b><p>Potential findings will appear here as authorized assessments produce evidence for review.</p></div></div>}
    </section>
  </div>;
}
