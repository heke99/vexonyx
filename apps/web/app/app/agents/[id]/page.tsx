...
export default async function AgentRunPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: run } = await ws.supabase
    .schema("ai")
    .from("agent_runs")
    .select("id,project_id,user_id,objective,state,current_step,max_steps,max_duration_seconds,max_tokens,max_tool_calls,max_cost,total_tokens,total_tool_calls,total_cost,model_alias,router_version,started_at,completed_at,created_at,updated_at")
    .eq("id", id)
  ...
  const [{ data: project }, { data: steps }, { data: checkpoints }, { data: approvals }] = await Promise.all([
    ...
    ws.supabase.schema("ai").from("agent_checkpoints").select("id,step_number,current_state,next_action,observation,usage_snapshot,budget_snapshot,created_at").eq("agent_run_id", id).eq("organization_id", ws.organizationId).order("step_number", { ascending: false }).limit(20),
    ws.supabase.schema("security").from("approval_requests").select("id,operation_type,status,requested_at,reviewed_at,expires_at,reason,decision_note").eq("agent_run_id", id).eq("organization_id", ws.organizationId).order("requested_at", { ascending: false }),
  ]);

  const latestApproval = approvals?.[0];
  const pending = approvals?.find((approval) => approval.status === "pending");
  const canAdvance = !terminalStates.has(run.state) && run.state !== "WAITING_FOR_APPROVAL";

  return <div className="app-content">
    <div className="app-heading"><div><div className="section-label">AGENT RUN / {run.state}</div><h1>{run.objective}</h1><p>{project?.name ?? "Project"} · progress is saved after every transition.</p></div><Link className="button button-small secondary" href="/app/agents">All runs →</Link></div>
    ...
    <section className="workspace-grid">
      ...
      <article className="workspace-card"><header><h2>Approval</h2><span>{pending ? "Pending" : latestApproval?.status ?? "Not required"}</span></header>{pending && isAdmin ? <><p className="form-note">Requested {new Date(pending.requested_at).toLocaleString("en-GB")} · expires {pending.expires_at ? new Date(pending.expires_at).toLocaleString("en-GB") : "not set"}</p><form className="auth-form" action={reviewAgentApproval}><input type="hidden" name="organization_id" value={ws.organizationId} /><input type="hidden" name="approval_id" value={pending.id} /><input type="hidden" name="run_id" value={id} /><label>Review note<input name="reason" maxLength={2000} placeholder="Optional reason" /></label><div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><button className="button" name="decision" value="approved" type="submit">Approve</button><button className="button secondary" name="decision" value="rejected" type="submit">Reject</button></div></form></> : pending ? <div className="empty-state"><div><b>Waiting for an organization admin.</b><p>The run cannot advance until the approval request is reviewed.</p></div></div> : latestApproval ? <div className="project-row"><div><b>{latestApproval.status}</b><small>{latestApproval.decision_note || latestApproval.reason || "No review note"}</small></div><span>{latestApproval.reviewed_at ? new Date(latestApproval.reviewed_at).toLocaleString("en-GB") : ""}</span></div> : <div className="empty-state"><div><b>No approval gate was required.</b><p>This preview can advance directly through its safe internal states.</p></div></div>}</article>
    </section>
    <section className="workspace-grid">
