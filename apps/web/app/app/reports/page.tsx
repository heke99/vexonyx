...
export default async function Page() {
  ...
  const ready = data?.filter((report) => String(report.status).toLowerCase() === "ready").length ?? 0;

  return <div className="app-content">
    <div className="app-heading"><div><h1>Reports</h1><p>Turn validated findings into versioned executive and technical reports without losing source evidence.</p></div><Link className="button button-small" href="/app/projects">Open projects</Link></div>
    ...
    <section className="workspace-card"><header><h2>Report library</h2><span>Versioned project output</span></header>
      {error ? <div className="empty-state"><div><b>Reports could not be loaded.</b><p>Try again. Existing report versions remain unchanged.</p></div></div> : data?.length ? data.map((report) => <Link className="project-row" href={`/app/reports/${report.id}`} key={report.id}><div><b>{report.title}</b><small>Updated {new Date(report.updated_at).toLocaleString("en-GB")}</small></div><span>{String(report.status).replaceAll("_", " ")} →</span></Link>) : <div className="empty-state"><div><b>No reports yet.</b><p>Create reports from a project after findings and evidence are ready for review.</p></div></div>}
    </section>
  </div>;
}
