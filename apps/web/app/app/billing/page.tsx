...
export default async function BillingPage() {
  ...
  const [subscription, plans, credits, account, transactions, entitlements, creditRates] = await Promise.all([
    ws.supabase.schema("billing").from("subscriptions").select("status,current_period_end,cancel_at_period_end,plan_id,plans(id,name,code)").eq("organization_id", ws.organizationId).maybeSingle(),
  ...
  return <div className="app-content">
    <div className="app-heading"><div><h1>Plan & credits</h1><p>Your plan, credits, limits and usage rates come from the same server-side catalog that Superadmin controls. Only provider-synced offers can appear here, and payment state is never inferred from the client.</p></div><BillingPortalButton /></div>
