...
export default async function BillingPage() {
  ...
  const [subscription, plans, credits, account, transactions] = await Promise.all([
    ...
    ws.supabase.schema("billing").from("plan_prices").select("id,billing_interval,currency,unit_amount_minor,active,provider_sync_status,plans!inner(id,name,code,description,is_public,status,display_order,provider_sync_status)").eq("active", true).eq("provider_sync_status", "synced").eq("plans.is_public", true).eq("plans.status", "active").eq("plans.provider_sync_status", "synced").order("unit_amount_minor"),
    ws.supabase.schema("billing").from("credit_products").select("id,code,name,description,credits,currency,unit_amount_minor,provider_sync_status").eq("active", true).eq("provider_sync_status", "synced").order("display_order"),
  ...
  return <div className="app-content">
    <div className="app-heading"><div><h1>Plan & credits</h1><p>Your plan, credits, invoices and limits use the same billing records that Superadmin sees. Only provider-synced offers can appear here, and payment state is never inferred from the client.</p></div><BillingPortalButton /></div>
