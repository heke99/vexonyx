...
export default async function Page() {
  ...
  const events: ActivityItem[] = [
    ...
    ...(findings.data ?? []).map((item) => ({ id:`finding-${item.id}`, label:"FINDING", title:item.title, detail:`${String(item.severity).toUpperCase()} · ${String(item.status).replaceAll("_", " ")}`, at:item.updated_at, href:`/app/projects/${item.project_id}` })),
    ...(runs.data ?? []).map((item) => ({ id:`run-${item.id}`, label:"AGENT", title:item.objective, detail:String(item.state).replaceAll("_", " ").toLowerCase(), at:item.updated_at, href:item.project_id ? `/app/projects/${item.project_id}` : "/app/agents" })),
  ...
  return <div className="app-content">
    ...
    <section className="workspace-card"><header><h2>Recent activity</h2><span>{events.length} updates</span></header>
      {hasError ? <div className="empty-state"><div><b>Activity could not be loaded.</b><p>Try again. No workspace data has been changed.</p></div></div> : events.length ? events.map((event) => <Link className="project-row" href={event.href ?? "/app"} key={event.id}><div><small>{event.label}</small><b>{event.title}</b><small>{event.detail} · {new Date(event.at).toLocaleString("en-GB")}</small></div><span>Open →</span></Link>) : <div className="empty-state"><div><b>No activity yet.</b><p>Project, file, finding and agent updates will appear here as work begins.</p></div></div>}
    </section>
  </div>;
}
