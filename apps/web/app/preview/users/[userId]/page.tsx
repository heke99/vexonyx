...
import styles from "@/components/customer-app.module.css";
import { WorkspaceDashboard } from "@/components/workspace-dashboard";
import { requireUserPreview } from "@/lib/admin/impersonation";
import { stopUserPreview } from "@/app/admin/user-preview-actions";
...
export default async function UserPreviewPage({ params }: { params: Promise<{ userId: string }> }) {
  ...
  const displayName = preview.targetDisplayName || preview.targetEmail;

  const previewBanner = <div className={styles.previewBanner}>
    <div><b>Viewing as {displayName}</b> · Customer view is read-only · Expires {new Date(preview.expiresAt).toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit" })}</div>
    <form action={stopUserPreview}><input type="hidden" name="user_id" value={targetUserId} /><button className={styles.previewExit} type="submit">Exit preview</button></form>
  </div>;

  return <CustomerAppShell readOnly displayName={displayName} preview={previewBanner}>
    <WorkspaceDashboard
      readOnly
      metrics={metrics}
