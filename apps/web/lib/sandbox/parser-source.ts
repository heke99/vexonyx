...
// Keep this source byte-for-byte behaviorally aligned with services/isolated-parser/parser.py.
// It is bundled into the trusted control plane so a deny-all sandbox never needs network access.
export const ISOLATED_PARSER_SOURCE = String.raw`#!/usr/bin/env python3
from __future__ import annotations
import argparse, io, json, re, tarfile, zipfile, zlib
...
def parse_pdf(data: bytes) -> dict:
    ...
    for m in re.finditer(rb'stream\\r?\\n(.*?)\\r?\\nendstream', data, re.S):
        ...
        elif b'/Filter' in prefix and b'/FlateDecode' not in prefix: continue
        chunks.extend(_extract_pdf_text_ops(stream))
        if sum(len(x) for x in chunks) >= MAX_TEXT: break
    if not chunks: chunks.extend(_extract_pdf_text_ops(data[:MAX_INPUT]))
...
def emit(payload: dict, output: str) -> None:
    ...
    if len(encoded.encode('utf-8')) > 10 * 1024 * 1024: raise Blocked('output_too_large')
    Path(output).write_text(encoded,encoding='utf-8')

def main() -> int:
    ap=argparse.ArgumentParser(); ap.add_argument('--input',required=True); ap.add_argument('--mime',default=''); ap.add_argument('--name',default=''); ap.add_argument('--output',required=True); args=ap.parse_args()
    path=Path(args.input).resolve()
    try:
        result=parse(path,args.mime,args.name or path.name); emit({'status':'ready',**result},args.output); return 0
    except Blocked as e:
        emit({'status':'blocked','error_code':str(e)},args.output); return 2
    except Exception as e:
        emit({'status':'failed','error_code':'parser_error','detail':type(e).__name__},args.output); return 1
