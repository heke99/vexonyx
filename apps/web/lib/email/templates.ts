    ...
    <tr>
      <td align="center">
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="max-width:600px;">
          <tr>
            <td style="padding:0 4px 22px;font-size:13px;font-weight:700;letter-spacing:.18em;color:${BRAND.accent};">${BRAND.name}</td>
          </tr>
          <tr>
            <td style="background:${BRAND.panel};border:1px solid #202831;border-radius:16px;padding:36px 32px;">
          ...
          <tr>
            <td style="padding:20px 4px 0;font-size:12px;line-height:1.6;color:${BRAND.faint};">${safeFootnote}<br />© 2026 VEXONYX</td>
          </tr>
        </table>
      </td>
    </tr>
...
export function waitlistVerificationTemplate(input: { verificationUrl: string; name?: string | null }): EmailTemplate {
  ...
  const preview = "Verify your email to finish joining the VEXONYX private beta waitlist.";
  return {
    subject: "Verify your email for VEXONYX",
    preview,
    text: `${input.name ? `Hi ${input.name},\n\n` : ""}Verify your email to finish joining the VEXONYX private beta waitlist.\n\n${input.verificationUrl}\n\nThis verification link expires in 30 minutes. If you did not request access, you can ignore this email.`,
    html: layout({
      preview,
      eyebrow: "PRIVATE BETA",
      title: "Verify your email",
      body: `<p style="margin:0 0 12px;">${greeting}</p><p style="margin:0;">Confirm this email address to complete your VEXONYX waitlist registration. You can join as an individual or as part of a company or team.</p>`,
      ctaLabel: "Verify email",
      ctaUrl: input.verificationUrl,
      footnote: "This verification link expires in 30 minutes. If you did not request access, you can safely ignore this message.",
    }),
  };
}
...
export function waitlistConfirmedTemplate(input: { name?: string | null; referralUrl?: string | null }): EmailTemplate {
  ...
  return {
    ...
    preview,
    text: `${input.name ? `Hi ${input.name},\n\n` : ""}Your email is verified and your place on the VEXONYX private beta waitlist is confirmed.${referralText}\n\nWe'll contact you when access is available.`,
    html: layout({
      preview,
      eyebrow: "WAITLIST CONFIRMED",
      title: "You're on the list",
      body: `<p style="margin:0 0 12px;">${greeting}</p><p style="margin:0;">Your email is verified and your VEXONYX private beta registration is confirmed. We’ll contact you when access becomes available.</p>${input.referralUrl ? `<p style="margin:16px 0 0;">Want to move up the list? Share your personal referral link.</p>` : ""}`,
      ctaLabel: input.referralUrl ? "View your referral link" : undefined,
      ctaUrl: input.referralUrl ?? undefined,
      footnote: "You received this email because this address was verified on the VEXONYX waitlist.",
    }),
  };
}
...
export function organizationInvitationTemplate(input: { organizationName: string; role: string; invitationUrl: string }): EmailTemplate {
  ...
  return {
    subject: `Join ${input.organizationName} on VEXONYX`,
    preview,
    ...
    html: layout({
      ...
      body: `<p style="margin:0;">You’ve been invited to join <strong style="color:${BRAND.text};">${organization}</strong> on VEXONYX as <strong style="color:${BRAND.text};">${role}</strong>. This invitation can only be accepted by this email address.</p>`,
      ctaLabel: "Accept invitation",
      ctaUrl: input.invitationUrl,
      footnote: "This invitation expires in seven days. If you were not expecting it, you can ignore this message.",
    }),
  };
}
